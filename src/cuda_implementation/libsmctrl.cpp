...
inline static void setup_sm_control_11() {
	...
	subscribe = (__typeof__(subscribe))subscribe_func_addr;
	enable = (__typeof__(enable))enable_func_addr;
	int res = 0;
...
inline void libsmctrl_set_global_mask(uint64_t mask) {
	int ver;
...
inline void set_sm_mask(uint64_t mask) {
