...
#include "spatial_multiplexing_scheduler.h"
...
SpatialMultiplexingScheduler::SpatialMultiplexingScheduler(int _gpu_id, 
                                                           ScheduleMode _mode, 
    ...
    LOG(INFO) << "SpatialMultiplexingScheduler: GPU ID = " << _gpu_id;
...
SpatialMultiplexingScheduler::~SpatialMultiplexingScheduler() {

}

Status SpatialMultiplexingScheduler::load_model_from_host_to_device(ModelID& mid) {
...
Status SpatialMultiplexingScheduler::bind_model_queue(
...
Status SpatialMultiplexingScheduler::run_multitask() {
    for(int i=0; i<rt_task_thread_pool.size();i++) {
        rt_task_thread_pool[i] = std::make_unique<std::thread>();
...
Status SpatialMultiplexingScheduler::run_orion() {
    ...
    for(int i=0; i<rt_task_thread_pool.size();i++) {
        rt_task_thread_pool[i] = std::make_unique<std::thread>();
        rt_task_thread_pool[i].reset(new std::thread([this] (int queue_idx) {
            this->cpu_core_manager.pin_multiple_cpu_core(5);
            ASSERT_GPU_ERROR(GPUCtxSetCurrent(missilebase::executor::GPUConfig::get_gpu_ctx()));
            // make sure the real_time queue is created for convenience.
            while (true) {
...
Status SpatialMultiplexingScheduler::run() {
    ...
    if (this->mode == MultiTaskPolicy) {
        return this->run_multitask();
    } else if (this->mode == OrionPolicy) {
        return this->run_orion();
    }
}

Status SpatialMultiplexingScheduler::shutdown() {
...
void SpatialMultiplexingScheduler::loop_body_rt_thread_multitask(int queue_id, std::shared_ptr<TaskQueue>& tqueue) {
...
void SpatialMultiplexingScheduler::loop_body_be_thread_multitask(int queue_id, std::shared_ptr<TaskQueue>& tqueue) {
    if (!tqueue->task_queue.empty()) {
...
bool SpatialMultiplexingScheduler::schedule_be_orion(
...
void SpatialMultiplexingScheduler::loop_body_gpu_launch_rt_kernel_orion() {
...
void SpatialMultiplexingScheduler::loop_body_gpu_launch_be_kernel_orion() {
    ...
    for(int round_idx=0; round_idx<be_task_gpu_launch_kernel_queues.size(); round_idx++) {
        ...
        if(be_task_gpu_launch_kernel_queues[qid]->empty() == false) {
            ...
            std::shared_ptr<SpatialMultiplexingExecutor> ptr_to_executor = 
                     std::dynamic_pointer_cast<missilebase::server::SpatialMultiplexingExecutor>(selected_be_task->model->executor);
            ASSERT_STATUS(ptr_to_executor->get_kernel_profile(selected_be_request->op_offset, 
                                                              selected_be_request->kernel_offset, 
                                                              be_kernel_profile));
            int has_rt_task_running_num = has_rt_task_running.load();
            if(has_rt_task_running_num == 0 || 
                      (has_rt_task_running_num>0 && 
                       be_kernel_profile.duration_ms < DUR_THRESHOLD*running_rt_task_total_latency.load())) {
                    need_launch_be_task = true;
                    selected_be_qid = qid;
                    break;
            } else if(schedule_be_orion(rt_kernel_profile, be_kernel_profile)) {
                need_launch_be_task = true;
                selected_be_qid = qid;
                break;
            }
...
void SpatialMultiplexingScheduler::loop_body_rt_thread_orion(int queue_id, std::shared_ptr<TaskQueue>& tqueue) {
    if (!tqueue->task_queue.empty()) {
        auto rt_task = tqueue->task_queue.front();
...
void SpatialMultiplexingScheduler::loop_body_be_thread_orion(int queue_id, std::shared_ptr<TaskQueue>& tqueue) {
    if (!tqueue->task_queue.empty()) {
        auto be_task = tqueue->task_queue.front();
        be_task->state = TaskState::Executing;
        execute_be_task(be_task, tqueue);
...
void SpatialMultiplexingScheduler::preempt_be_tasks(int num_required_TPCs, 
    ...
    for (int i = 0; i < be_task_thread_pool.size(); i++) {
        ...
        if (!be_queue.empty()) {
            ...
            int be_task_used_num_TPCs = be_task->model->executor->get_used_num_TPCs();
                ...
                << num_required_TPCs << " / be_queue[" 
                << i << "] / be_task_used_num_TPCs = " << be_task_used_num_TPCs;
            #endif
...
void SpatialMultiplexingScheduler::resume_be_tasks(std::shared_ptr<Task> &be_task_to_be_preempted) {
    //if (preempted) return;
    #ifdef DEBUG_MODE
...
Status SpatialMultiplexingScheduler::wait_launch_kernel_complete(int launch_kernel_queue_id, bool is_real_time) {
    if(is_real_time) {
...
void SpatialMultiplexingScheduler::execute_rt_task(std::shared_ptr<Task> &task, std::shared_ptr<TaskQueue> &rt_queue) {
    #ifdef DEBUG_MODE
...
void SpatialMultiplexingScheduler::execute_be_task(std::shared_ptr<Task>& task, std::shared_ptr<TaskQueue>& be_queue) {
    #ifdef DEBUG_MODE
