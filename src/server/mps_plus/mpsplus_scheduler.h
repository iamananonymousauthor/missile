...
class MPSPlusScheduler : public BaseScheduler {
public:
    MPSPlusScheduler(int _gpu_id, ScheduleMode _mode, int _max_num_rt_queues = 64, int _max_num_be_queues = 16);
    ~MPSPlusScheduler();
...
private:
    ...
    std::unique_ptr<std::thread> gpu_launch_rt_kernel_thread, 
                                 gpu_launch_be_kernel_thread, 
                                 gpu_launch_memcpy_htod_thread, 
                                 gpu_launch_memcpy_dtoh_thread;

    executor::GPUStream_t rt_gpu_launch_kernel_stream, be_gpu_launch_kernel_stream;
    executor::GPUStream_t memcpy_htod_stream, memcpy_dtoh_stream;
    ...
    void loop_body_htod_copy();
    void loop_body_dtoh_copy();
