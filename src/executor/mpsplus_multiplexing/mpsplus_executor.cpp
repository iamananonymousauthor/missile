...
MPSPlusExecutor::MPSPlusExecutor() {
    is_preempted = false;
}

MPSPlusExecutor::~MPSPlusExecutor() {}

Status MPSPlusExecutor::load_param_from_host_to_device() {
...
Status MPSPlusExecutor::load_model_from_GPU_module(const char* json_file_path, 
...
Status MPSPlusExecutor::init_executor(const char* json_file_path, 
                                      GPUModule_t module, 
    ...
    Status MPSPlusExecutor::set_input(
            const std::string& key, const std::vector<float>& value) {
        return set_input(key, (void*)value.data(), value.size() * sizeof(float));
    }

    Status MPSPlusExecutor::set_input(const std::string& key, const void* value, size_t len) {
    ...
    Status MPSPlusExecutor::set_input(int idx, const void* value, size_t len) {
    ...
    Status MPSPlusExecutor::get_output(std::vector<float>& out) {
        size_t input_storage_idx;
    ...
    Status MPSPlusExecutor::get_output(void* out, size_t len) {
        size_t input_storage_idx;
        if (find_storage_idx("output", input_storage_idx) != Status::Succ) RETURN_STATUS(Status::NotFound);
        StorageInfo& storage_info = this->model->storage[input_storage_idx];
        size_t storage_size = Model::get_stype_size(storage_info.stype) * storage_info.size;
    ...
    Status MPSPlusExecutor::get_data(int idx, void* out, size_t len) {
        if (idx >= model->storage.size()) RETURN_STATUS(Status::OutOfRange);
        StorageInfo& storage_info = this->model->storage[idx];
        size_t storage_size = Model::get_stype_size(storage_info.stype) * storage_info.size;
        if (len < storage_size) RETURN_STATUS(Status::Fail);
    ...
    Status MPSPlusExecutor::execute_to_create(int &create_step_offset) {
    ...
    Status MPSPlusExecutor::execute_to_swap_in(int &swap_in_step_offset, GPUStream_t stream) {
        for(; swap_in_step_offset < step_graph->swap_in_step_node_list.size(); swap_in_step_offset++) {
            ...
            auto request = std::make_shared<GPULaunchMemcpyRequest>((void*)this->storage[storage_idx].load(),
                                                    (void*)array.h_ptr,
                                                    Model::get_stype_size(storage_info.stype) * storage_info.size);
            this->scheduler->memcpy_htod_queues[this->is_real_time][this->qid]->push(request);
            step_graph->swap_in_step_node_list[swap_in_step_offset]->mark_finished();
        }
    }

    Status MPSPlusExecutor::execute_to_swap_out(int &swap_out_step_offset, GPUStream_t stream) {
        for(; swap_out_step_offset < step_graph->swap_out_step_node_list.size(); swap_out_step_offset++) {
            ...
            GPU_RETURN_STATUS(GPUMemcpyDtoHAsync(
                    (void*)array.h_ptr, (GPUDevicePtr_t)this->storage[storage_idx].load(),
                    Model::get_stype_size(storage_info.stype) * storage_info.size, stream));
    ...
    Status MPSPlusExecutor::execute_to_launch(int &launch_step_offset, GPUStream_t stream, 
        ...
        for(; launch_step_offset < step_graph->launch_step_node_list.size(); launch_step_offset++) {
            ...
            if (is_preempted) {
#ifdef DEBUG_MODE
                LOG(INFO) << "DEBUG: batch job is preempted! TPC_request_num_cores = " << TPC_request_num_cores;
#endif
                return Status::Succ;
            }
            ...
            this->scheduler->memcpy_htod_queues[this->is_real_time][this->qid]->wait_until_empty();
            ...
            for (int kernel_idx = 0; kernel_idx < model->ops[op_idx].kernels.size(); kernel_idx++) {
                ...
                if (this->scheduler->mode == server::MPSPlusPolicy) {
    ...
    Status MPSPlusExecutor::execute_to_reclaim(int &reclaim_step_offset) {
    ...
    Status MPSPlusExecutor::execute_to(GPUStream_t stream, int TPC_request_num_cores, int TPC_limit_num_cores) {
        if(is_preempted) {
            return Status::Succ;
        }
        ...
        launch_thread.reset(new std::thread([this] (GPUStream_t stream, int TPC_request_num_cores, int TPC_limit_num_cores) {
            ASSERT_GPU_ERROR(GPUCtxSetCurrent(missilebase::executor::GPUConfig::get_gpu_ctx()));
        ...
        if(step_graph->is_finished(create_step_offset, swap_in_step_offset,
            ...
            LOG(INFO) << (is_real_time?"RT":"BE") 
                      << " finished an iteration! time elapsed in launch kernel: " 
                      << time_elapsed_launch_kernel_ms 
                      <<" ms";
        }
...
Status MPSPlusExecutor::launch_kernel(int op_offset, 
                                      int kernel_offset, 
                                      GPUStream_t stream, 
                                      int TPC_request_num_cores, 
                                      int TPC_limit_num_cores) {
...
Status MPSPlusExecutor::get_kernel_profile(int op_offset, int kernel_offset, KernelProfile &kernel_profile){
    std::string kernel_name = this->model->ops[op_offset].kernels[kernel_offset].name;
    if(map_kernel_name_to_kernel_profile.find(kernel_name) == map_kernel_name_to_kernel_profile.end()) {
        return Status::InvalidArgument;
    }
    kernel_profile = map_kernel_name_to_kernel_profile[kernel_name];
    return Status::Succ;
}
} // namespace executor
} // namespace missilebase