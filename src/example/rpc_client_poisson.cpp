...
// Adapted from DISB Benchmark
// Reference: https://github.com/SJTU-IPADS/disb/blob/d61059f10c616581d649f3f40cc53c277ce74b64/src/load.cpp#L154
std::chrono::microseconds expRandInterval_us(double sleep_time_ms, std::default_random_engine &random_engine, std::uniform_real_distribution<double> &uniform_distribution)
{
    // The time interval between two events in the Poisson process
    // satisfies the exponential distribution.
    double u = uniform_distribution(random_engine); // u is in interval [0, 1]
    double interval_ms = (-sleep_time_ms) * log(u);
    return std::chrono::microseconds(int64_t(interval_ms * THOUSAND));
}
...
void func_task_submit(int sleep_time_ms, int duration_time_sec, std::shared_ptr<TaskQueue> &task_queue){
    ...
    while (true) {
        ...
        std::this_thread::sleep_for(expRandInterval_us((double)sleep_time_ms, random_engine, uniform_distribution));
    }
}

int main(int argc, char** argv) {
    if (argc != 14) {
        std::cerr << "Usage: " << std::string(argv[0]) << " port model_dir model_name [real_time] [num_sm_request] [num_sm_limit] [alloc_memory_size_kib] [perc_pcie_bandwidth] [avg_sleep_time(ms)] [duration(s)] [estimated_runtime_ms] [slo_ms] [record_file_path]\n";
    ...
    int sleep_time = std::atoi(argv[9]);
    ...
    std::cout << "Duration time: " << duration_time_sec << "sec, Sleep time: " << sleep_time << " ms" << std::endl;
    ...
    task_submit_thread.reset(new std::thread(func_task_submit, sleep_time, duration_time_sec, std::ref(task_queue)));
